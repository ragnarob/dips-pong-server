...
module.exports = {
  setupRoutes () {
    app.get('/api/players', async (req, res) => {
      ...
      if (!req.query || !req.query.officeId) {
        allPlayers = await this.getAllPlayersAllOffices()
      }
      else {
        allPlayers = await this.getAllPlayers(req.query.officeId)
      }
      res.json(allPlayers)
    })

    app.get('/api/players/:name', this.hasOfficeIdInQuery, async (req, res) => {
      let playerStats = await this.getPlayerStats(req.query.officeId, req.params.name) 
      res.json(playerStats)
    })

    app.delete('/api/players/:id', authorize,  async (req, res) => {
      await this.deletePlayer(req.params.id)
      res.json({success: true})
    })

    app.post('/api/players', authorize, this.hasOfficeIdInBody, async (req, res) => {
      await this.addPlayer(req.body.officeId, req.body.newPlayerName)
      res.json({success: true})
    })

    app.post('/api/players/:id', authorize, async (req, res) => {
  ...
  async getAllPlayersAllOffices () {
    let query = 'SELECT player.id AS id, player.name AS name, elo, office.id AS officeId, office.name AS officeName, COUNT(*) AS gamesCount FROM player INNER JOIN office ON (player.office = office.id) LEFT JOIN game ON (game.winner = player.id OR game.loser = player.id) GROUP BY player.id ORDER BY officeId ASC, elo DESC'
    let allPlayers = await databaseFacade.execute(query)
    return allPlayers
  },
  ...
  async getPlayerStats (officeId, playerName) {
    ...
    let [playerResult, matchesResult] = await Promise.all([
      databaseFacade.execute(playerQuery, playerQueryParams),
      databaseFacade.execute(matchesQuery, matchesQueryParams)
    ])
